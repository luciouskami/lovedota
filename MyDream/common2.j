...
//===================================================
// Custom  defs     by      MentalDease
//===================================================   
    constant        integer     VK_HOME  =   36
    constant        integer     VK_END   =   0x23
    constant        integer     VK_DEL   =   0x2E   //钩不动的目标  
    constant        integer     VK_TAB   =  9          //切换目标    
    constant        integer     VK_INSERT = 0x2D   //钩移动的目标
    
    ///////======ORDER_ID
    constant        integer     ORDER_BLINK = 0xD022D
    constant        integer     POM_JUMP     = 0xD00A1
    constant        integer     PUDGE_HOOK  = 0xD0278
    constant        integer     ORDER_MOVING      = 851971
    constant        integer     ORDER_P        = 851991
    constant        integer     PUDGE_FIRE_ON = 0xD00D1
    constant        integer     PUDGE_FIRE_OFF = 0xD00D2
    constant        integer     PUDGE_YAO    = 0xD0200
    constant        integer     NAGA_FEN_SHEN = 0xD009B
    
     
    timer           DotaGlobalTimer = null      //count time
    timer           GameStartTimer  = null      //game  start
    integer         GameModeDetect  = 0         //game  mode
    constant        boolean     bDebugState  = true   //debuging?
    unit    array   AllHeros                    //AllHeros
    real    array   AllHPs                      //the hps of All heros
    timer           PomArrowTimer   = null
    constant        integer	POM_ARROW_UNIT_TYPE_ID = 1747988533   //Pom  
    constant        integer PUDGE_HOOK_TYPE_ID = 1966092360     //TF 
    constant        integer SHOOTER_TYPE_ID = 1697657170        //枪 
    boolean         bPomArrowOn = false
    boolean         bPomArrowCreate = false
    boolean         bPudegHookOn = false
    real            lastPudgeHook = 0
    real            lastHootKey = 0
    boolean         bShowAllPlace = false
    timer           ShowAllPlaceTimer = null
    
    boolean         bFuckShooterOn = false
    unit            ShooterUnit = null
    unit            HeroSniper   = null
    real            snipermana = 0
    
    //英雄ID 
    constant        integer SNIPER_HERO_ID = 1433631084     //火枪 
    constant        integer ES_HERO_ID = 1333027688         //小牛
    
    unit            HeroES  = null      //小牛
    real            YANG_DAO_RANGE = 800    //羊刀施法距离 
    
    
    constant        integer TIAO_DAO_TYPE_ID = 1227895880    //跳刀 
    constant        integer BKB_TYPE_ID = 1227900506        //BKB 
    constant        integer FEN_SHEN_TYPE_ID = 1227897158    //分身斧
    constant        integer CHUI_FEN_TYPE_ID = 1227896922    //吹风
    constant        integer YANG_DAO_TYPE_ID = 1227899212    //羊刀
    constant        integer TUI_TUI_TYPE_ID = 1227901001     //推推
    constant        integer YIN_DAO_TYPE_ID = 1227897160     //隐刀
    constant        integer REN_JIA_TYPE_ID = 1227896911     //刃甲
    constant        integer DI_ZI_TYPE_ID = 1227901750      //笛子 
    
    integer         DotaTimeCount_002 = 0
    integer         DotaTimeCount_01 = 0
    integer         DotaTimeCount_02 = 0
    integer         DotaTimeCount_05 = 0
    integer         DotaTimeCount_1 = 0

    real Latency = 0.15


    unit PomArrowUnit = null
    real PomArrowHurtrange = 200
    real PudgeHookHurtrange = 200
    
    real autoHookTime = 0
    real LastChangeTime = 0
    integer tempAutoTarget = 0
    real    LastAutoX = 0
    real    LastAutoY = 0
    boolean IsTfFireOn = false
    boolean IsTfYaoOn  = false
...
native DestroyTimerMD         takes timer whichTimer returns nothing
native TimerStartMD           takes timer whichTimer, real timeout, boolean periodic, code handlerFunc returns nothing
...
constant native GetSpellAbilityIdMD           takes nothing returns integer
...
constant native GetEventPlayerChatStringMD takes nothing returns string
...
constant native GetTriggerUnitMD takes nothing returns unit
...
// Unit API
// Facing arguments are specified in degrees
native          CreateUnitMD              takes player id, integer unitid, real x, real y, real face returns unit
native          CreateUnitByName        takes player whichPlayer, string unitname, real x, real y, real face returns unit
native          CreateUnitAtLocMD         takes player id, integer unitid, location whichLocation, real face returns unit
...
native          SetUnitOwnerMD        takes unit whichUnit, player whichPlayer, boolean changeColor returns nothing
...
native UnitAddAbilityMD             takes unit whichUnit, integer abilityId returns boolean
native UnitRemoveAbilityMD          takes unit whichUnit, integer abilityId returns boolean
...
native DisplayTimedTextToPlayerMD     takes player toPlayer, real x, real y, real duration, string message returns nothing
...
native AddSpecialEffectTargetMD     takes string modelName, widget targetWidget, string attachPointName returns effect
...
//=======================================================
//      CUSTOM  NATIVES
native KeyIsPressedMD       takes integer keyCode                       returns boolean
native WriteFileStringMD    takes string fileName, string content       returns nothing
native IsTempReplayReadyMD  takes nothing                               returns boolean
native GlobalGetIntMD       takes string key                            returns integer
native GlobalSetIntMD       takes string key, integer value             returns nothing     
native GetArrayDwordMD      takes string arrayName, integer index       returns integer
native I2UMD                takes integer intUnit                       returns unit    //Integer2Unit
native TargetOrderIssueMD   takes integer orderId, item usedItem, real x, real y, widget targetWidget, integer option1, integer option2 returns nothing
native Widget2UnitMD        takes widget  src                           returns unit
native UseItemNoLocMD       takes integer slotid, item usedItem, integer opt1, integer opt2 returns nothing
native UseItemLocMD         takes integer slotid, item usedItem, real x, real y, integer opt1, integer opt2 returns nothing
native ClearArrayDataMD     takes nothing                               returns nothing
...
function RMod takes real dividend, real divisor returns real
...
function Square takes real a returns real
  return a * a
endfunction

function DebugStringMD  takes string msg returns nothing
    if  bDebugState == true then
        call DisplayTimedTextToPlayerMD(GetLocalPlayer(),0,0,10,msg)
    endif
endfunction

function ShowStringMD takes string msg returns nothing
    call DisplayTimedTextToPlayerMD(GetLocalPlayer(),0,0,10,msg)
endfunction


function GetGlobalTime takes nothing returns real
  return TimerGetElapsed(DotaGlobalTimer)
endfunction

function IsDotaGame takes nothing returns boolean
  if VersionGet() != VERSION_FROZEN_THRONE then
    return false
  endif

  if IsTempReplayReadyMD() == false then
    return false
  endif
  
  if GameModeDetect == 0 then
    if SubString(GetLocalizedString("TRIGSTR_50000"), 0, 4)=="DotA" then
      set GameModeDetect = 2
    else
      set GameModeDetect = 1
    endif
  endif
  return (GameModeDetect == 2)
endfunction

function IsUnitAliveMD takes unit whichUnit returns boolean
    return (GetUnitState(whichUnit, UNIT_STATE_LIFE) > 0)
endfunction

function GetHeroTurnTime takes unit hero, real angle returns real
  local real result =0
  //set result =  (angle/180) / ( GetUnitTurnSpeed(hero) * 6 )  + 0.05
  set result = 0.0035352 * angle + 0.08
  //call OutputScreen(R2S(result))
  return result

endfunction


function DistanceBetweenHeroAndLine takes real directionDegree, real pointX, real pointY , unit hero returns real
  local real arrowFacing = 0.01745329 * directionDegree
  local real a = Cos(arrowFacing)
  local real b = Sin(arrowFacing)
  local real ax = pointX
  local real ay = pointY
  local real hx = GetUnitX(hero)
  local real hy = GetUnitY(hero)
  return RAbs(b * hx - a * hy + a * ay - b * ax)
endfunction

function DistanceBetweenHeroAndArrowLine takes unit arrow, unit hero returns real
  return DistanceBetweenHeroAndLine(GetUnitFacing(arrow), GetUnitX(arrow), GetUnitY(arrow), hero)
endfunction

function DistancePt2Pt takes real x1, real y1, real x2, real y2 returns real
    return SquareRoot(Pow(x1-x2, 2)+Pow(y1-y2, 2))
endfunction

function DistanceUnit2Unit takes unit u1, unit u2 returns real
    return DistancePt2Pt(GetUnitX(u1),GetUnitY(u1),GetUnitX(u2),GetUnitY(u2))
endfunction

function IsComputerUnitOnDamageLine takes real arrowangle, real x, real y, real hurtrange, boolean canHurtAlly returns boolean
    return false
endfunction

function GetVectorAngle takes real x, real y returns real
  //local real modular = SquareRoot(x * x + y * y)
  //return Acos(x / modular)
  return RMod(Atan2(y,x), 6.2831853)
endfunction

function SelectMyHero takes unit hero returns nothing
    if IsUnitSelected(hero,GetLocalPlayer())==false then
        call ClearSelection()
...
//逃跑 
function HeroFleeDirectionDecideXY takes real direction, real x, real y, unit hero returns real
  local real arrowFacing = 0.01745329 * direction
  local real arrowX = Cos(arrowFacing)
  local real arrowY = Sin(arrowFacing)
  local real arrowHeroX = GetUnitX(hero) - x
  local real arrowHeroY = GetUnitY(hero) - y
  local real fleeAX = Cos(arrowFacing + 1.570796)
  local real fleeAY = Sin(arrowFacing + 1.570796)
  local real fleeBX = Cos(arrowFacing - 1.570796)
  local real fleeBY = Sin(arrowFacing - 1.570796)
  local real product1 = 0
  local real product2 = 0

  //running
  if GetUnitCurrentOrder(hero)==0xD0003 then
    set arrowHeroX = arrowHeroX + 54 * Cos(GetUnitFacing(hero) * 0.0174)
    set arrowHeroY = arrowHeroY + 54 * Sin(GetUnitFacing(hero) * 0.0174)
  endif

  if (arrowX * arrowHeroX + arrowY * arrowHeroY < 0) then
    return -1.0
  endif
  
  set product1 = arrowX * arrowHeroY - arrowY * arrowHeroX


  set product2 = arrowHeroX * fleeAY - arrowHeroY * fleeAX

  if (product1 * product2 > 0) then
    return GetVectorAngle(fleeAX, fleeAY)
  else
    return GetVectorAngle(fleeBX, fleeBY)
  endif
endfunction

function HeroFleeDirectionDecide takes unit arrow, unit hero returns real
  return HeroFleeDirectionDecideXY(GetUnitFacing(arrow), GetUnitX(arrow), GetUnitY(arrow), hero)
endfunction

function IssueHeroMoveTowardDirection takes unit u, real directionRAD, real distance, integer flag1, integer flag2 returns nothing
  local real x = 0
  local real y = 0

  if u==null or directionRAD<0 or GetUnitState(u,UNIT_STATE_LIFE)<=0 or IsUnitVisible(u,GetLocalPlayer())==false then
    return
  endif
  
  //选中了没 
  if IsUnitSelected(u,GetLocalPlayer())==false then
    call ClearSelection()
    call SelectUnit(u,true)
  endif
  set x = GetUnitX(u)+Cos(directionRAD)*distance
  set y = GetUnitY(u)+Sin(directionRAD)*distance

  //RClick
  call TargetOrderIssueMD(0xD0003,null,x,y,null,flag1,flag2)
...
function PomArrowTimerAction takes nothing returns nothing
    local real myx
    local real myy
    local real arrowfacing
    local real arrowx
    local real arrowy
    local player ownPlayer
    local unit myHero
    local real distancetoline
    local real fleeDirectionRAD
    local integer firstArrow = 0
    
    if bPomArrowOn == false then
        return
    endif
    
    set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
    
    if  PomArrowUnit!=null and IsUnitAliveMD(PomArrowUnit) and IsUnitEnemy(PomArrowUnit, GetLocalPlayer())==true then //enemy pom
        set myx = GetUnitX(myHero)
        set myy = GetUnitY(myHero)
        set arrowfacing = GetUnitFacing(PomArrowUnit)
        set arrowx = GetUnitX(PomArrowUnit)
        set arrowy = GetUnitY(PomArrowUnit)
        if  bPomArrowCreate == true then
            call PingMinimapEx(arrowx,arrowy,2.0,100,100,100,true)
            set bPomArrowCreate = false
            set firstArrow = 1
        endif
        if DistancePt2Pt(myx, myy, arrowx, arrowy) < 3000 and IsUnitAliveMD(myHero)==true then
            set distancetoline = DistanceBetweenHeroAndArrowLine(PomArrowUnit, myHero)
            if firstArrow == 1 then
                call ShowStringMD("|CFFFF0000POM ARROW!!")
            endif
            if PomArrowHurtrange > distancetoline then //may hurt me
                if IsComputerUnitOnDamageLine(arrowfacing,arrowx,arrowy,140,false)==false then //no units on damage line
                    set fleeDirectionRAD = HeroFleeDirectionDecide(PomArrowUnit, myHero)
                    if fleeDirectionRAD >= 0 then
                        //flee
                        call ShowStringMD("|CFFFF0000RUN AWAY!!")
                        call IssueHeroMoveTowardDirection(myHero, fleeDirectionRAD, PomArrowHurtrange-distancetoline, 6, 0)
                        set PomArrowUnit = null
                    endif
                endif
            endif
        endif
    else
        call DebugStringMD("pom end")
        set PomArrowUnit = null 
    endif
                    
endfunction

function PudgeHookAction takes unit pudgeHook returns nothing
    local real myx
    local real myy
    ...
    if bPudegHookOn == false then
        return
    endif
    
    set tmpTime = GetGlobalTime()
    if  tmpTime - lastPudgeHook <= 5 then
        return
    endif
    
    //call DebugStringMD(R2S(GetUnitMoveSpeed(pudgeHook)))
    
    set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
    
    if  IsUnitEnemy(pudgeHook, GetLocalPlayer())==true then //enemy pudge
        set myx = GetUnitX(myHero)
        set myy = GetUnitY(myHero)
        set hookfacing = GetUnitFacing(pudgeHook)
        set hookx = GetUnitX(pudgeHook)
        set hooky = GetUnitY(pudgeHook)
        
        call PingMinimapEx(hookx,hooky,1.0,100,100,100,true)
        if DistancePt2Pt(myx, myy, hookx, hooky) < 1200 and IsUnitAliveMD(myHero)==true then
            set distancetoline = DistanceBetweenHeroAndArrowLine(pudgeHook, myHero)
            if PudgeHookHurtrange > distancetoline then //may hurt me
                if IsComputerUnitOnDamageLine(hookfacing,hookx,hooky,125,false)==false then //no units on damage line
                    set fleeDirectionRAD = HeroFleeDirectionDecide(pudgeHook, myHero)
                    if fleeDirectionRAD >= 0 then
                        //flee
                        call ShowStringMD("|CFFFF0000RUN AWAY!!")
                        call IssueHeroMoveTowardDirection(myHero, fleeDirectionRAD, PudgeHookHurtrange-distancetoline, 6, 0)
                        set lastPudgeHook = GetGlobalTime()
                    endif
                endif
            endif
        endif
...
//技能-不需要目的地 向前跳 
function UseNoLocSkillEscape takes unit hero, integer orderId returns nothing
    call SelectMyHero(hero)
    call UseItemNoLocMD(orderId, null, 0, 4)
endfunction

//技能-需要目的地　向前跳300 
function UseLocSkillEscape takes unit hero, integer orderId returns nothing
    local real directionRAD = GetUnitFacing(hero)
    local real x = GetUnitX(hero) + Cos(directionRAD)*300
    local real y = GetUnitY(hero) + Sin(directionRAD)*300
    call SelectMyHero(hero)
    call UseItemLocMD(orderId, null, x, y, 0, 4)
endfunction

//物品-不要需目的地
function UseNoLocItemEscape takes unit hero, integer orderId, item usedItem returns nothing
    call SelectMyHero(hero)
    call UseItemNoLocMD(orderId, usedItem, 0, 4)
endfunction

//物品-需要目的地　向前跳300
function UseLocItemEscape takes unit hero, integer orderId, item usedItem returns nothing
    local real directionRAD = GetUnitFacing(hero)
    local real x = GetUnitX(hero) + Cos(directionRAD)*300
    local real y = GetUnitY(hero) + Sin(directionRAD)*300
    call SelectMyHero(hero)
    call UseItemLocMD(orderId, usedItem, x, y, 0, 4)
endfunction
 
//用技能
function UseSkillEscape takes unit hero returns boolean
    //call DebugStringMD("AM---A0KY="+I2S(GetUnitAbilityLevel(hero, 'A0KY'))+"A022="+I2S(GetUnitAbilityLevel(hero,'A022'))+"AEbl="+I2S(GetUnitAbilityLevel(hero,'AEbl')))
    //call DebugStringMD("POM--A0L8="+I2S(GetUnitAbilityLevel(hero, 'A0L8'))+"A0LN="+I2S(GetUnitAbilityLevel(hero,'A0LN'))+"A0KV="+I2S(GetUnitAbilityLevel(hero,'A0KV'))) 
    //if GetUnitAbilityLevel(hero, 'A0LN') > 0 then //月女跳躲不掉。。。。 
    //    call DebugStringMD("POM") 
    //    call UseNoLocSkillEscape(hero, POM_JUMP)  //无目的地 
    //    return true
    //endif
    if GetUnitAbilityLevel(hero, 'A0ME') > 0 then //女王闪烁
        call DebugStringMD("QOP") 
        call UseLocSkillEscape(hero, ORDER_BLINK) //有目的地 
        return true
    endif
    if GetUnitAbilityLevel(hero, 'AEbl') > 0 then //敌法闪烁
        call DebugStringMD("AM") 
        call UseLocSkillEscape(hero, ORDER_BLINK) //有目的地 
        return true
    endif
    //if GetUnitAbilityLevel(hero, 'A063') >0 then  //那加分身--来不及了。。 
//        call DebugStringMD("naga")
//        call UseNoLocSkillEscape(hero, NAGA_FEN_SHEN)
//        return true
//    endif      
    return false
endfunction
...
//用物品
function UseItemEscape takes unit hero returns boolean
    local integer i = 0
    local item tmp
    local integer typeId
    loop
        exitwhen (i > 5)
        set tmp = UnitItemInSlot(hero, i)
        set typeId = GetItemTypeId(tmp)
        if typeId == TIAO_DAO_TYPE_ID then    //跳刀 
            call DebugStringMD("tiao dao")
            call UseLocItemEscape(hero, i+0xD0028, tmp)
            //return true
        endif
        if typeId == BKB_TYPE_ID then           //BKB 
            call DebugStringMD("bkb")
            call UseNoLocItemEscape(hero, i+0xD0028, tmp)
            //return true
        endif
        if typeId == FEN_SHEN_TYPE_ID then       //分身 
            call DebugStringMD("fenshen")
            call UseNoLocItemEscape(hero, i+0xD0028, tmp)
            //return true
        endif
        if typeId == CHUI_FEN_TYPE_ID then      //吹风 
            call DebugStringMD("吹风")
            call SelectMyHero(hero)
            call TargetOrderIssueMD(i+0xD0028, tmp, GetUnitX(hero), GetUnitY(hero), hero, 0, 4) 
        endif
      //  if typeId == TUI_TUI_TYPE_ID then       //推推
      //      call DebugStringMD("推推")
      //      call TargetOrderIssueMD(i+0xD0028, tmp, GetUnitX(hero), GetUnitY(hero), hero, 0, 4)
      //  endif 
       // if typeId == YIN_DAO_TYPE_ID then       //隐刀
      //      call DebugStringMD("隐刀")
      //      call UseNoLocItemEscape(hero, i+0xD0028, tmp)
     //   endif
        if typeId == REN_JIA_TYPE_ID then       //刃甲
            call DebugStringMD("刃甲")
            call UseNoLocItemEscape(hero, i+0xD0028, tmp)
        endif 
        if typeId == DI_ZI_TYPE_ID then       //笛子 
            call DebugStringMD("笛子")
            call UseNoLocItemEscape(hero, i+0xD0028, tmp)
        endif 
        set i = i+1            
    endloop
...
function FuckShooter takes nothing returns nothing
    local real tmpmana = 0.1
    local unit myHero
    if  bFuckShooterOn == false then
        return
    endif

    //非空　还在射我　火枪是是活的 
    if ShooterUnit!= null and IsUnitAliveMD(ShooterUnit)==true and IsUnitAliveMD(HeroSniper) then 
        //call DebugStringMD("shooting me")
        set tmpmana = GetUnitState(HeroSniper, UNIT_STATE_MANA)
        if tmpmana < snipermana then //火枪射出来了 
            //call DebugStringMD("need jmp")
            set ShooterUnit = null
            set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
            if UseSkillEscape(myHero)== true then   //先用技能闪 
                //return
            endif
            if UseItemEscape(myHero) == true then   //再用物品闪 
                //return
            endif
        endif
    else
...
function TestHook   takes unit hero returns nothing
    local real directionRAD = GetUnitFacing(hero)
    local real x = GetUnitX(hero) + Cos(directionRAD)*300
    local real y = GetUnitY(hero) + Sin(directionRAD)*300
    call SelectMyHero(hero)
    call TargetOrderIssueMD(PUDGE_HOOK, null, x, y, null, 6, 4)
endfunction

function TestFire   takes unit hero returns nothing
    call SelectMyHero(hero)
    call UseItemNoLocMD(PUDGE_FIRE_ON, null, 0, 2)
endfunction

function TestYao    takes unit hero returns nothing
    local unit target
    set target = AllHeros[tempAutoTarget]
    call SelectMyHero(hero)
    call TargetOrderIssueMD(PUDGE_YAO, null, GetUnitX(target), GetUnitY(target), target, 0, 4)
endfunction 

//tf使用烧 
function UseTfSkillFire  takes nothing returns nothing
    //A06K
    local unit myHero
    local unit target
    //call DebugStringMD("fire")
    if tempAutoTarget == 0 then
        return
    endif
    set target = AllHeros[tempAutoTarget]
    set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
    if IsUnitAliveMD(target)==false then
        set IsTfFireOn = false
        return
    endif
    if IsTfFireOn == true and DistanceUnit2Unit(myHero,target) < 350 then //距离小250
        set IsTfFireOn = false
        call SelectMyHero(myHero)
        call UseItemNoLocMD(PUDGE_FIRE_ON, null, 0, 2)
    endif
endfunction

//TF咬人
function UseTfSkillYao  takes nothing returns nothing
    //A0FL
    local unit myHero
    local unit target
    if tempAutoTarget == 0 then
    ...
    if IsTfYaoOn == true and DistanceUnit2Unit(myHero,target) < 88 then //敌方 距离小88
        set IsTfYaoOn = false
        call SelectMyHero(myHero)
        call TargetOrderIssueMD(PUDGE_YAO, null, GetUnitX(target), GetUnitY(target), target, 0, 4)
    endif
...
//钩移动的目标 
function UseSkillHookMoving   takes nothing returns nothing
    local real tmpTime = 0
    local unit tmpUnit
    local unit myHero
    local integer hookLen = 0
    local integer hookLvl = 0
    local real  x1
    local real  y1
    local real  x2
    local real  y2
    ...
    local real  cosa
    local real  sina
    if  KeyIsPressedMD(VK_INSERT)==true then
        set tmpTime = GetGlobalTime()
        if tmpTime - autoHookTime < 0.6 then
            call DebugStringMD("Cool Down")
            return
        else
            set autoHookTime = tmpTime
        endif
        if tempAutoTarget == 0 then
            call ShowStringMD("No Target")
            return
        endif
        set tmpUnit = AllHeros[tempAutoTarget]
        set myHero =  AllHeros[GetPlayerId(GetLocalPlayer())]
        if  IsUnitEnemy(tmpUnit, GetLocalPlayer()) == false then     //not enemy
            call ShowStringMD("Not A Good Target")
            return
        else
            set hookLvl = GetUnitAbilityLevel(myHero, 'A06I')
            if hookLvl == 0 then
                call ShowStringMD("Hook Ability Level = 0")
                return
            endif
            
            set x1 = GetUnitX(myHero)
            set y1 = GetUnitY(myHero)
            set x2 = GetUnitX(tmpUnit)
            set y2 = GetUnitY(tmpUnit)
            set facing = GetUnitFacing(tmpUnit)
            set v2 = GetUnitMoveSpeed(tmpUnit)
            set s = 0
            set cosa = Cos(0.01745329*facing)
            set sina = Sin(0.01745329*facing)
            set speeder = v1/v2
            set hookLen = 40*(5+6*hookLvl-1)
            loop
                exitwhen(s>1000)
                set x3 = s*cosa+x2
                set y3 = s*sina+y2
                if RAbs(DistancePt2Pt(x2,y2,x3,y3)*speeder-DistancePt2Pt(x1,y1,x3,y3)-388) < 10 then //可以钩 -修正 
                    if DistancePt2Pt(x1,y1,x3,y3) > I2R(hookLen) then //太长了
                        call ShowStringMD("Distance Too Long")
                    else
                        call SelectMyHero(myHero)
                        call TargetOrderIssueMD(PUDGE_HOOK, null, x3, y3, null, 6, 4)
                        set IsTfFireOn = true  //开烧 
                        set IsTfYaoOn = true    //开咬
                    endif
                    return
                endif
                set s = s+10
...
//钩不动的目标 
function UseSkillHookNotMoving   takes nothing returns nothing
    local real tmpTime = 0
    local unit tmpUnit
    local unit myHero
    local integer hookLen = 0
    local integer hookLvl = 0
    if  KeyIsPressedMD(VK_DEL)==true then
        set tmpTime = GetGlobalTime()
        if tmpTime - autoHookTime < 0.6 then
            return
        else
        ...
        if  IsUnitEnemy(tmpUnit, GetLocalPlayer()) == false then     //not enemy
            ...
            set hookLen = 40*(5+6*hookLvl-1)
            if DistanceUnit2Unit(myHero, tmpUnit) > I2R(hookLen) then
                call ShowStringMD("Distance Too Long")
                return
            endif
            call SelectMyHero(myHero)
            call TargetOrderIssueMD(PUDGE_HOOK, null, GetUnitX(tmpUnit), GetUnitY(tmpUnit), null, 6, 4)
            set IsTfFireOn = true  //开烧 
            set IsTfYaoOn = true    //开咬 
        endif                       
    endif     
...
function DotaTimerAction_002 takes nothing returns nothing
    if PomArrowUnit != null then  //POM 
        call  PomArrowTimerAction()
    endif 
    if ShooterUnit != null then //火枪
        call  FuckShooter()
    endif
    
    call UseSkillHookNotMoving()   //不动的 
    call UseSkillHookMoving()   //移动的 
    call UseTfSkillYao()
    call UseTfSkillFire()
...
function DotaGetAllHeros takes nothing  returns nothing
    local   integer i = 1
    local   integer HeroId = 0
    loop
        exitwhen (i > 11)
        set HeroId = GetArrayDwordMD("L1", i)
        if  HeroId != 0 then
           // call    DebugStringMD("Name"+I2S(i)+"="+GetUnitName(I2UMD(HeroId)))
            set AllHeros[i] = I2UMD(HeroId)
            if GetUnitTypeId(AllHeros[i])==ES_HERO_ID and IsUnitEnemy(AllHeros[i],GetLocalPlayer()) then    //敌方小牛 
                set HeroES = AllHeros[i]
            endif
        else
            set AllHeros[i] = null
        endif
...
function ShowAllPlaceAction takes nothing returns nothing
    local integer loopi = 0
    local unit    tmpunit
    if IsDotaGame() == true then
        loop
            exitwhen (loopi > 11)
            set tmpunit = AllHeros[loopi]
            //非空　敌人　生活着的。。。。 
            if  tmpunit!=null and IsUnitEnemy(tmpunit, GetLocalPlayer())==true and IsUnitAliveMD(tmpunit)==true then
                call PingMinimapEx(GetUnitX(tmpunit),GetUnitY(tmpunit),1.0,254,0,0,true)
            endif
            set loopi = loopi + 1
        endloop
    endif    
...
function GetAnEnemyHero takes nothing returns integer
    local integer loopi = 0
    local unit    tmpunit
    if IsDotaGame() == true then
        loop
...
//切换目标 
function ChangeHookTgt takes nothing returns nothing
    local real tmpTime
    
    set tmpTime = GetGlobalTime()
    
    if tmpTime - LastChangeTime < 0.6 then
        return
    else
        set LastChangeTime = tmpTime
    endif
    
    if tempAutoTarget == 0 then
        set tempAutoTarget = GetAnEnemyHero()
        call ShowStringMD("Now--"+GetUnitName(AllHeros[tempAutoTarget]))
        return
    endif
    
    if tempAutoTarget == 5 then
        set tempAutoTarget = 1
        call ShowStringMD("Now--"+GetUnitName(AllHeros[tempAutoTarget]))
        return
    endif
    if tempAutoTarget == 11 then
        set tempAutoTarget = 7
        call ShowStringMD("Now--"+GetUnitName(AllHeros[tempAutoTarget]))
        return
    endif
    
    set tempAutoTarget = tempAutoTarget +1
    call ShowStringMD("Now--"+GetUnitName(AllHeros[tempAutoTarget]))

endfunction

function GetUnitStatePercentMD takes unit whichUnit, unitstate whichState, unitstate whichMaxState returns real
...
function ShowDamageAndHeal takes nothing returns nothing
    local real tmpHP = 0
    local real percentHP = 0
    local real lastHP = 0
    local real diff = 0
    ...
    loop
        exitwhen(i>11)
        set tmpHero = AllHeros[i]
        set lastHP = AllHPs[i]
        set tmpHP = GetUnitState(tmpHero, UNIT_STATE_LIFE)
        //活着的　可见的　2400内的 
        if IsUnitAliveMD(tmpHero) and IsUnitVisible(tmpHero, GetLocalPlayer()) and (Square(GetCameraTargetPositionX()-GetUnitX(tmpHero))+Square(GetCameraTargetPositionY()-GetUnitY(tmpHero)) < Square(2400)) then
            set diff = tmpHP - lastHP
            if diff>40 and lastHP!=0 then //回血 40
                set percentHP = GetUnitStatePercentMD(tmpHero, UNIT_STATE_LIFE, UNIT_STATE_MAX_LIFE)
                set tt = CreateTextTag()
                call SetTextTagText(tt, "＋"+I2S(R2I(diff))+"  "+I2S(R2I(percentHP))+"%", 0.026)
                call SetTextTagPos(tt, GetUnitX(tmpHero)-48, GetUnitY(tmpHero), 0)
            ...
            if diff<0 and lastHP!=0 then //掉血
                set percentHP = GetUnitStatePercentMD(tmpHero, UNIT_STATE_LIFE, UNIT_STATE_MAX_LIFE)
                set tt = CreateTextTag()
                call SetTextTagText(tt, "－"+I2S(R2I(diff))+"  "+I2S(R2I(percentHP))+"%", 0.026)
                call SetTextTagPos(tt, GetUnitX(tmpHero)-48, GetUnitY(tmpHero), 0)
...
//有没小牛 自动变羊  
function DetectHeroES takes nothing returns nothing
    local unit myHero
    local item tmpItem
    local integer i = 0
    if HeroES == null or bPomArrowOn==false then
        return
    endif
    set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
    if DistanceUnit2Unit(myHero, HeroES) < YANG_DAO_RANGE and UnitHasItemType(HeroES, TIAO_DAO_TYPE_ID) then //牛有跳 距离小于800
        loop
            exitwhen(i>5)
            set tmpItem = UnitItemInSlot(myHero, i)
            if GetItemTypeId(tmpItem)==YANG_DAO_TYPE_ID then    //羊刀 
                call SelectMyHero(myHero)
                call TargetOrderIssueMD(i+0xD0028, tmpItem, GetUnitX(HeroES), GetUnitY(HeroES), HeroES, 0, 4)
            endif
            if GetItemTypeId(tmpItem)== CHUI_FEN_TYPE_ID then   //吹风 
                call SelectMyHero(myHero)
                call TargetOrderIssueMD(i+0xD0028, tmpItem, GetUnitX(HeroES), GetUnitY(HeroES), HeroES, 0, 4)
            endif
            if GetItemTypeId(tmpItem)==DI_ZI_TYPE_ID then  //笛子
                call SelectMyHero(myHero)
                call UseNoLocItemEscape(myHero, i+0xD0028, tmpItem) 
            endif
            set i=i+1
        endloop
        set HeroES = null //临时置0 
    endif
endfunction  
      
        

function DetectHotKey takes nothing returns nothing
    local real tmpTime = 0
    set tmpTime = GetGlobalTime()
    if tmpTime - lastHootKey <= 1.0 then
        return
    endif       
    if KeyIsPressedMD(VK_HOME) == true then  //月女箭
        set lastHootKey = GetGlobalTime()    //更新时间 
        set bPomArrowOn = not bPomArrowOn
        set bPudegHookOn = not bPudegHookOn
        set bFuckShooterOn = not bFuckShooterOn
        if  bPomArrowOn == true then
            call ShowStringMD("PomArrow detect ON")
        else
            call ShowStringMD("PomArrow detect OFF")
        endif
        if  bPudegHookOn == true then
            call ShowStringMD("PudgeHook detect ON")
        else
            call ShowStringMD("PudgeHook detect OFF")
        endif
        if  bFuckShooterOn == true then
            call ShowStringMD("Fuck Shooter ON")
        else
            call ShowStringMD("Fuck Shoooter OFF")
        endif
    endif
    if KeyIsPressedMD(VK_END) == true then  //All Places
        set lastHootKey = GetGlobalTime()   //更新时间 
        set bShowAllPlace = not bShowAllPlace
        if  bShowAllPlace == true then
            call ShowStringMD("Show All Heros Place ON")
        else
            call ShowStringMD("Show All Heros Place OFF")
        endif
    endif
    
    if KeyIsPressedMD(VK_TAB) == true then  //切换目标 
        call ChangeHookTgt()
    endif
endfunction
           
        
function DotaTimerAction takes nothing returns nothing
    local   integer i = 0
    local   real ptx
    local   real pty
    
    call DetectHeroES()                //检测小牛 
    
    
    set DotaTimeCount_002 = DotaTimeCount_002 + 1
    if DotaTimeCount_002 == 2 then //2*0.01
      set DotaTimeCount_002 = 0
      call DotaTimerAction_002()                //钩子　箭　火枪 
      call DetectHotKey()                        //Hot Key
    endif
    
    set DotaTimeCount_01 = DotaTimeCount_01 + 1
    if DotaTimeCount_01 == 10 then  //0.1
        set DotaTimeCount_01 = 0               //显伤害
        call ShowDamageAndHeal() 
    endif
     
    set DotaTimeCount_1 = DotaTimeCount_1 + 1
    if  DotaTimeCount_1 == 100 then              //1 
        set DotaTimeCount_1 = 0
        call DotaGetAllHeros()
        if  bShowAllPlace == true then          //show all enemy places
            call  ShowAllPlaceAction()
        endif
    endif 
       
endfunction

function ShowAllPlayersInfo takes nothing returns nothing
     call  DebugStringMD("showallplayerinfo")
endfunction

function DotaInit   takes nothing returns nothing
    //Set globals values
    local integer loopi = 0
    loop
        exitwhen(loopi>11)
        set AllHeros[loopi] = null
        set loopi = loopi+1
    endloop
    
    set HeroES = null
    
    call ClearArrayDataMD()
endfunction


//得到火枪Unit 
function GetHeroSniper takes nothing returns unit
    local unit tmpunit = null
    local integer i = 1
    loop
        exitwhen (i>11)
        set tmpunit = AllHeros[i]
        if GetUnitTypeId(tmpunit) == SNIPER_HERO_ID then
            return tmpunit
        endif
        set i = i+1
    endloop
...
//检测Event
function DetectEvent takes nothing returns nothing
endfunction 
...
//      Hooked Functions
//=======================================================
function DisplayTimedTextToPlayer     takes player toPlayer, real x, real y, real duration, string message returns nothing
    call  DisplayTimedTextToPlayerMD(toPlayer, x, y, duration, message)
endfunction

function TimerStart takes timer whichTimer, real timeout, boolean periodic, code handlerFunc returns nothing
    if  IsDotaGame() == true and timeout == 120.0 and periodic == false and handlerFunc!=null then
        //call DebugStringMD("timer start")
        call DotaInit()
        call TimerStartMD(whichTimer, 0.01, true, function DotaTimerAction)
        return
    endif
    if IsDotaGame()==true and timeout == 99999.0 and periodic==false and handlerFunc==null then
      set DotaGlobalTimer = whichTimer
    endif
    
    
    if handlerFunc != null then
        call TimerStartMD(whichTimer, timeout, periodic, handlerFunc)
    else
        call TimerStartMD(whichTimer, timeout, periodic, null)
    endif
endfunction

function DestroyTimer takes timer whichTimer returns nothing
  if GameStartTimer==null then
    set GameStartTimer = whichTimer
  endif
  call DestroyTimerMD(whichTimer)
endfunction

function CreateUnit takes player id, integer unitid, real x, real y, real face returns unit
    local unit newunit
    
    set newunit = CreateUnitMD(id, unitid, x, y, face)
    if  GetUnitTypeId(newunit) == POM_ARROW_UNIT_TYPE_ID then
        set bPomArrowCreate = true
        set PomArrowUnit = newunit
    endif
    if  GetUnitTypeId(newunit) == PUDGE_HOOK_TYPE_ID then
        call PudgeHookAction(newunit)
    endif
    if  GetUnitTypeId(newunit) == SHOOTER_TYPE_ID then
        if id == GetLocalPlayer() then  //狙我
            set HeroSniper = GetHeroSniper()
            if HeroSniper != null and IsUnitEnemy(HeroSniper, GetLocalPlayer())== true then //找到敌方火枪
                call DebugStringMD("SHOOTER"+GetPlayerName(id))
                set snipermana = GetUnitState(HeroSniper, UNIT_STATE_MANA) //获取当前MANA 
                set ShooterUnit = newunit
            endif
        endif
    endif
    //call DebugStringMD("CreateUnit"+I2S(GetUnitTypeId(newunit)))
    return newunit
endfunction

function SetUnitOwner takes unit whichUnit, player whichPlayer, boolean changeColor returns nothing
    call    SetUnitOwnerMD(whichUnit, whichPlayer, changeColor)
endfunction


function ShowSkillLevel takes unit hero returns nothing
    //tf
    call DebugStringMD("A06I="+I2S(GetUnitAbilityLevel(hero,'A06I'))+" A06D="+I2S(GetUnitAbilityLevel(hero,'A06D'))+" A0FL="+I2S(GetUnitAbilityLevel(hero,'A0FL'))+" A06K="+I2S(GetUnitAbilityLevel(hero,'A06K')))    
    //naga
    call DebugStringMD("A00E="+I2S(GetUnitAbilityLevel(hero,'A00E'))+" A0BA="+I2S(GetUnitAbilityLevel(hero,'A0BA'))+" A063="+I2S(GetUnitAbilityLevel(hero,'A063'))+" A07U="+I2S(GetUnitAbilityLevel(hero,'A07U')))  
endfunction

function TestImageCreate takes unit hero returns nothing
    local real size
    local real rng = 200
    local image img
    set size = rng*2.13
    set img = CreateImage( "ReplaceableTextures\\Selection\\SpellAreaOfEffect_basic.blp", size,size,size,GetUnitX(hero),GetUnitY(hero),0,size/2+12,size/2+12,0,2)
    
    call SetImageColor(img, 0, 66, 255, 192)
    call SetImageRenderAlways(img, true)
endfunction

function GetEventPlayerChatString takes nothing returns string
    local string chatwords
    local integer i = 0
    local item  tmp
    local unit myHero
    set chatwords = GetEventPlayerChatStringMD()
    set myHero = AllHeros[GetPlayerId(GetLocalPlayer())]
    if  chatwords == "-showinfo" then
        call ShowAllPlayersInfo()
    endif
    if  chatwords == "-showheroid" then
        call DebugStringMD(I2S(GetUnitTypeId(AllHeros[1])))
    endif
    if  chatwords == "-showslot" then
        loop
            exitwhen (i > 5)
            set tmp = UnitItemInSlot(myHero, i)
            call DebugStringMD(I2S(i)+" -- " + I2S(GetItemTypeId(tmp)))
            set i = i+1
        endloop
    endif
    if  chatwords == "-testitem" then
        call UseItemEscape(myHero)
    endif
    if  chatwords == "-testskill" then
        call UseSkillEscape(myHero)
    endif
    if  chatwords == "-getfacing" then
        call DebugStringMD(R2S(GetUnitFacing(myHero)))
    endif
    if  chatwords == "-getorder" then
        call DebugStringMD(I2S(GetUnitCurrentOrder(myHero)))
    endif
    if  chatwords == "-testhook" then
        call TestHook(myHero)
    endif
    if  chatwords == "-showskilllevel" then
        call ShowSkillLevel(myHero)
    endif
    if  chatwords == "-testfire" then
        call TestFire(myHero)
    endif
    if  chatwords == "-testyao" then
        call TestYao(myHero)
    endif
    if  chatwords == "-gettgtorder" then
        call DebugStringMD(I2S(GetUnitCurrentOrder(AllHeros[tempAutoTarget])))
    endif
    if  chatwords == "-showpos" then
        call DebugStringMD("x="+R2S(GetUnitX(myHero))+" y="+R2S(GetUnitY(myHero)))
    endif 
    if  chatwords == "-testimg" then
        call TestImageCreate(myHero)
    endif
    
    return  chatwords
endfunction

function UnitAddAbility takes unit whichUnit, integer abilityId returns boolean
    return UnitAddAbilityMD(whichUnit, abilityId)
endfunction

function UnitRemoveAbility takes unit whichUnit, integer abilityId returns boolean
//    if abilityId == 1110455880 then
//        call DebugStringMD("removeAbillity")
//    endif
    return  UnitRemoveAbilityMD(whichUnit, abilityId)
endfunction


function AddSpecialEffectTarget  takes string modelName, widget targetWidget, string attachPointName returns effect
    return  AddSpecialEffectTargetMD(modelName, targetWidget, attachPointName)
endfunction

function CreateUnitAtLoc  takes player id, integer unitid, location whichLocation, real face returns unit
    local unit newunit
    set newunit = CreateUnitAtLocMD(id, unitid, whichLocation, face)
    //call DebugStringMD("CreateUnitAtLoc uid="+I2S(unitid) + " typeid=" + I2S(GetUnitTypeId(newunit)))
    //call WriteFileStringMD("c:\\creepslog.txt", I2S(GetUnitTypeId(newunit)))
    return newunit
endfunction

function GetSpellAbilityId  takes nothing returns integer
    call DetectEvent()
    return GetSpellAbilityIdMD()
endfunction

function GetTriggerUnit takes nothing returns unit
    call DetectEvent()
    return GetTriggerUnitMD()
endfunction    
